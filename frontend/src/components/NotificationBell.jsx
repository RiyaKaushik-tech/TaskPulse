...
const NotificationBell = () => {
  ...
  // Fix: Access user object correctly from Redux state
  ...
  const fetchUnreadCount = async () => {
    try {
      const endpoint = isAdmin ? "/logs/admin" : "/logs/user/unread-count";
      console.log("Fetching from endpoint:", endpoint, "isAdmin:", isAdmin);
  ...
  const fetchRecentEvents = async () => {
    try {
      const endpoint = isAdmin ? "/logs/admin" : "/logs/user";
  ...
  const handleViewAll = () => {
    ...
    const route = isAdmin ? "/admin/activity-logs" : "/users/notifications";
    console.log("Navigating to:", route, "isAdmin:", isAdmin);
    navigate(route);
  };
  ...
  return (
    <div className="relative">
      ...
      >
       <FaBell className="text-lg mt-1.5"/>
        {unreadCount > 0 && (
        ...
        <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
          <div className="p-3 border-b border-gray-200 flex items-center justify-between">
            <h3 className="font-semibold text-gray-900">
              {isAdmin ? "Activity Logs" : "Notifications"}
            </h3>
            {unreadCount > 0 && (
          ...
          <div className="max-h-96 overflow-y-auto">
            {recentEvents.length === 0 && (
              <p className="p-4 text-sm text-gray-500 text-center">
                {isAdmin ? "No activity logs" : "No notifications"}
              </p>
            )}
          ...
          <div className="p-3 border-t border-gray-200">
            ...
            >
              {isAdmin ? "View All Activity Logs" : "View All Notifications"}
            </button>
          </div>
        </div>
      )}
